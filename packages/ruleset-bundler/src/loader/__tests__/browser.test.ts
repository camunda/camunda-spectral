...
describe('bundleAndLoadRuleset browser', () => {
  ...
  beforeEach(() => {
    ...
    serveAssets({
      'https://esm.sh/lodash.lowercase': `export * from "/v135/lodash.lowercase@4.3.0/es2022/lodash.lowercase.mjs";
export { default } from "/v135/lodash.lowercase@4.3.0/es2022/lodash.lowercase.mjs";`,
...
export { default } from "/v135/lodash.uppercase@4.3.0/es2022/lodash.uppercase.mjs";`,
      ...
      'https://esm.sh/v135/lodash.lowercase@4.3.0/es2022/lodash.lowercase.mjs':
      ...
      '/p/.spectral/my-fn.js': `import {isOdd} from './helpers/index.js';
...
export default (input) => {
  if (!isOdd(input)) {
    return [{ message: 'must be odd' }];
      ...
      '/p/.spectral/upper-case.js': `import upperCase from 'lodash.uppercase';
...
export default (input) => {
  if (upperCase(input) !== input) {
    return [{ message: 'must be upper case' }];
      ...
      '/p/.spectral/lower-case.js': `import lowerCase from 'https://esm.sh/lodash.lowercase';
...
export default (input) => {
  if (lowerCase(input) !== input) {
    return [{ message: 'must be lower case' }];
      ...
      '/p/.spectral/helpers/index.js': `export * from './is-odd.js';`,
      '/p/.spectral/helpers/is-odd.js': `export const isOdd = (value) => value % 2 === 1`,
...
export default {
  rules: {
    ...
    'lower-case-rule': {
      ...
      '/p/spectral.json': JSON.stringify(
        {
          functionsDir: '.spectral',
          functions: ['my-fn', 'upper-case', 'lower-case'],
          rules: {
            'odd-rule': {
              given: '$',
              then: { function: 'my-fn' },
            ...
            'upper-case-rule': {
              given: '$',
              then: { function: 'upper-case' },
            },
            'lower-case-rule': {
              given: '$',
              then: { function: 'lower-case' },
            },
          },
        },
        null,
  ...
  it('should be able to load JS ruleset', async () => {
    const ruleset = await bundleAndLoadRuleset('/p/spectral.js', io);
    ...
    expect(JSON.parse(JSON.stringify(ruleset))).toStrictEqual({
      ...
      source: '/p/spectral.js',
    });
  });

  it('should be able to load JSON/YAML ruleset', async () => {
    const ruleset = await bundleAndLoadRuleset('/p/spectral.json', io);
